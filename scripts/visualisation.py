    ...
    try:
        ...
        if 'DTW_Distance' in combined_data.columns:
            ...
            for i, sector in enumerate(sectors):
                ...
                sns.lineplot(ax=axes[i], data=sector_data, x='Date', y='DTW_Distance', color='red')
                axes[i].set_title(f'DTW Distances Over Time for {sector}')
                axes[i].set_xlabel('Date')
                axes[i].set_ylabel('DTW Distance')
        ...
        plt.title('Correlation Heatmap of Closing Prices Across Sectors')
        plt.tight_layout()
        ...
        for i, sector in enumerate(sectors):
            ...
            sns.lineplot(ax=axes[i], data=sector_data, x='Date', y='Volatility')
            axes[i].set_title(f'Volatility Over Time for {sector}')
            axes[i].set_xlabel('Date')
            axes[i].set_ylabel('Volatility (Rolling Std Dev)')
            axes[i].tick_params(axis='x', rotation=45)
        plt.tight_layout()
